...
namespace Proyecto_escuela.Clases
{
    class Cls_Salida : Cls_conexion
    {
        ...
        {
            grid.Rows.Clear();
            string query = "SELECT foto_perfil, tb_alumnos.id_alumno, matricula, nombres, apellidos FROM tb_alumnos Inner JOIN tb_salida on tb_alumnos.id_alumno=tb_salida.id_alumno WHERE id_estatus_salida='2' LIMIT 3";
            MySqlConnection databaseConnection = new MySqlConnection(connectionString);
            MySqlCommand commandDatabase = new MySqlCommand(query, databaseConnection);
            commandDatabase.CommandTimeout = 60;
            MySqlDataReader reader;
